...
__device__ void printTree(cuaTree* raiz) {
...
//Device
__global__ void printPunTree(universo* uni) {
...
//HASTA AQUI TODO ES PERFECTO Y REVISADO

//FUNCIONES DEVICE:
//device dentro de rellenaTree: Device
...
//device dentro de preLiberaTree
...
__global__ void cleanMatrizTree(universo* d_uni) { //<< < SIZE, SIZE >> >
	//punTree
...
void iterateUniverseTreeGPU(universo* uni, int nSegundos, bool print) {
	...
	//Crear espacio para Uni y Tree en Device
	universo* d_uni;
	...
	while (timeLeft >= TIMELAPSE) {
		...
		if (countTree == CLEANTREEITERATION) {
			...
			//Libera trabaja en Device de manera reiterativa: Ha de ser device, introducirlo en un global
			//cleanmatrizTree trabaja en device de manera normal, que sea global, e introducir liberatree en él 
