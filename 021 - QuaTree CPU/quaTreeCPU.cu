...
#define N 100					//N�mero de Cuerpos en el universo
#define TIMELAPSE 3600			//N�mero de segundos que pasan entre instantes
#define G 6.67428/pow(10, 11)	//Constante G
#define MAXDIM 15*pow(10, 8)	//Rango de posici�n en X e Y
#define MAXSPEED 3*pow(10,3)	//Rango de velocidad en X e Y
#define MAXMASS 6*pow(10,24)	//Masa m�xima de un Cuerpo
#define MINMASS 1*pow(10,23)	//Masa minima de un Cuerpo
#define nNiveles 8				//N�mero de niveles del arbol. 
#define SIZE pow(2,nNiveles)	//N�mero de filas y columnas de la matriz de ramas final
#define CLEANTREEITERATION 1	//N�mero de iteraciones antes de actualizar el arbol
...
//Si no uso punteros a los niveles m�s bajos, tendr� que nadar a traves de las ramas en ambas direcciones. 
...
// y el hijo de ese hijo. Todo para un tree que est� m�s cerca que su hermano del cuadrante 4
//Por ello, punteros. 
//Problema: 
//Soluci�n: 
...
void ramificaTree(cuaTree* raiz, universo* uni) {
	...
	cuaTree* rama;
	bool ramificarMas = (raiz[0].nivelTree + 1 < nNiveles);//Es el nivel que creo ahora el nivel m�s bajo?
	...
	if (nCuerposRama1 > 0) {
		...
		raiz[0].rama1 = rama;
		//Si no es un nivel suficiente, se ramifica m�s
		if (ramificarMas) {
