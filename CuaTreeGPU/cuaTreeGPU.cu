...
//te rellena un cuatree malloc-eado. 
...
void rellenaRamaGPU(universo* uni, cuaTree** punTreeGPU, int** punCuerposGPU) {
	int nFilas = pow(2, nNiveles);
	struct cuaTree* rama;
...
void elProceso(universo* uni, universo* d_uni, cuaTree* raiz, cuaTree** punTreeGPU, int** punCuerposGPU, cuaTree** d_punTreeGPU, int** d_punCuerposGPU) {
	int nFilas = pow(2, nNiveles);
	...
	rellenaRamaGPU(uni, punTreeGPU, punCuerposGPU);
	cudaMemcpy(d_punTreeGPU, punTreeGPU, sizeof(cuaTree*) * nCasillas, cudaMemcpyHostToDevice);
	cudaMemcpy(d_punCuerposGPU, punCuerposGPU, sizeof(int*) * nCasillas, cudaMemcpyHostToDevice);
...
__device__ void forceIterate(universo* uni, int idCuerpo1, int idCuerpo2) {
...
__device__ void calculoFuerzaCuerpoTree(universo* uni, int idCuerpo, cuaTree* rama) {
...
__global__ void forceIterateTree(universo* d_uni, cuaTree** d_punTreeGPU, int** d_punCuerposGPU) {
	...
	if (puntTree == NULL) {
		...
		for (int i = 0; i < nCuerpos; i++) {
			for (int j = i + 1; j < nCuerpos; j++) {
				forceIterate(d_uni, puntTree->cuerpos[i], puntTree->cuerpos[j]);
...
void newForcesTree(universo* d_uni, cuaTree** d_punTreeGPU, int** d_punCuerposGPU) {
	...
	forceIterateTree << <nFilas, nColumnas >> > (d_uni, d_punTreeGPU, d_punCuerposGPU); //Invocamos muchos threads
...
void iterateUniverseTreeGPU(universo* uni, int nSegundos, bool print) {
	...
	struct cuaTree** d_punTreeGPU;
	cudaMalloc(&d_punTreeGPU, sizeof(cuaTree*) * nCasillas);

	int** punCuerposGPU = (int**)malloc(sizeof(int*) * nCasillas);
	int** d_punCuerposGPU;
	cudaMalloc(&d_punCuerposGPU, sizeof(int*) * nCasillas);
	...
	raiz = new cuaTree;
	raizTreeGPU(uni, raiz);
	ramificaTree(raiz, uni);	//Ahora mismo, uni->punTreeMatriz contiene todos los punteros con los hijos
	...
	//Relleno uni->punTreeMatrizGPU y punCuerposGPU sin modificar lo que hay en punTreeMatriz
	//printf("RellenaRamaGPU finaliza\n");
	//Paso el universo actual a GPU
	...
	while (timeLeft >= TIMELAPSE) {
		...
		*/
		...
		//LIBERAR Y CREAR EL TREE
		if (countTree == CLEANTREEITERATION) {
			//Si han pasado x iteraciones, Libero toda la GPU menos d_uni, paso d_uni a uni, limpio la raiz, la recreo, relleno d_uni
			elProceso(uni, d_uni, raiz, punTreeGPU, punCuerposGPU, d_punTreeGPU, d_punCuerposGPU);
			countTree = 0;
	...
	printf("Termino la funcion de IterarUniversoTreeGPU\n");
}
